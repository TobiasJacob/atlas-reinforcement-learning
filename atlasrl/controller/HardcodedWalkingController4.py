...
while (1):
    ...
    # Calculating given forces except external wrenches
    # (31, 6) = (31, 6, 6) @ (31, 6, 36) @ (36) + (31, 6, 6) @ (31, 6, 36) @ (36) + (31, 6) * (31, 6, 36) @ (36)
    vDot0 = jacobianDot @ qDot # + jacobian @ qDotDot # (31, 6)
    inertiaForces0 = -(I @ vDot0[:, :, None])[:, :, 0] # (31, 6)
    inertias0 = (inertiaForces0[:, None, :] @ rootJacobian).sum(0)[0] # (6,)
    gravityMoment = (FGrav[:, None, :] @ rootJacobian).sum(0)[0] # (6,)

    # Here comes the interesting part.
    # For correct dynamics the following equation holds:
    # ((FWrench + FGrav + inertiaForces0 + inertiaForces1) @ rootJacobian).sum(0) = 0
    # FWrench can be chosen for any foot on the ground. inertiaForces1 satisfies
    # inertiaForces1 = (-I @ (jacobian @ qDotDot)[:, :, None])[:, :, 0] # (31, 6)
    # But qDotDot also has to satisfy the inverse kinematics problem
    # For all iEnd: jacobian[1+iEnd] @ qDotDot = vDotDot[1+iEnd]
    ...
    qDotDot = np.concatenate((qDotDotRootJoint, qDotDotUpperBody, qDotDotLegs)) # (36,)
    ...
    Wb = -gravityMoment - inertiasQDot # (6,)

    # Inequality constraints for wrenchs
    Fwrench = np.zeros((31, 6))
    if currentFoot == "b":
        ...
        lBack = 0.1
        l = 0.1
        # technically it is not -F_z * l < M_x < F_z * l
        # but ... < M_x < F_z * l * F_z / sqrt(F_x ** 2 + F_z ** 2)
        BA = np.array(([
        ...
        if res["status"] == "optimal":
            wrenchs = np.array(res["x"])[:, 0]
            # print(wrenchs)
        else:
            print("Infeasible")
            # exit(0)
            pass
        Fwrench[1+iLeft] = wrenchs[:6]
        Fwrench[1+iRight] = wrenchs[6:]
    elif currentFoot == "l":
        wrenchs = np.linalg.solve(WA[:, :6], Wb)
        Fwrench[1+iLeft] = wrenchs
    elif currentFoot == "r":
        wrenchs = np.linalg.solve(WA[:, 6:], Wb)
        Fwrench[1+iRight] = wrenchs
    else:
        raise NotImplementedError()
    # wrenchs @ WA.transpose() - Wb = 0
    # =>
    # FWrench @ rootJacobian + FGrav @ rootJacobian + inertiaForces @ rootJacobian = 0
    # Calculating joint torques
    totalForce = inertiaForces + FGrav + Fwrench # (31, 6)
