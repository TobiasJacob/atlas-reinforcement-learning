...
def process_ref_joint_pos_data(joint_pos):
    ...
    for i in range(num_pos):
        ...
        curr_pos = curr_pos * REF_POS_SCALE + REF_POS_OFFSET
        proc_pos[i] = curr_pos
...
def retarget_root_pose(ref_joint_pos):
    ...
    forward_dir += FORWARD_DIR_OFFSET
    ...
    root_rot = transformations.quaternion_multiply(root_rot, INIT_ROT)
...
def retarget_pose(robot, default_pose, ref_joint_pos):
    ...
    for i in range(len(REF_TOE_JOINT_IDS)):
        ...
        sim_hip_id = SIM_HIP_JOINT_IDS[i]
        toe_offset_local = SIM_TOE_OFFSET_LOCAL[i]
    ...
    joint_pose = pybullet.calculateInverseKinematics2(robot, SIM_TOE_JOINT_IDS,
                                                        tar_toe_pos,
...
def retarget_motion(robot, joint_pos_data):
    ...
    for f in range(num_frames):
        ...
        curr_pose = retarget_pose(robot, DEFAULT_JOINT_POSE, ref_joint_pos)
...
def main():
    ...
    while True:
        for mocap_motion in mocap_motions:
            ...
            robot = pybullet.loadURDF(URDF_FILENAME, INIT_POS, INIT_ROT)
            # Set robot to default pose to bias knees in the right direction.
            set_pose(robot, np.concatenate([INIT_POS, INIT_ROT, DEFAULT_JOINT_POSE]))
